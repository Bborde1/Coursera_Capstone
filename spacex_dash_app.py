...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        fig = px.pie(filtered_df, names='class', values='class', title=f"Total Success Launches for site {entered_site}")
        return fig
# TASK 4:
...
def get_scatter_plot(entered_site, payload_mass_range):
    if entered_site == 'ALL':
        filtered_df2 = spacex_df.loc[(spacex_df['Payload Mass (kg)'] >= payload_mass_range[0]) & (spacex_df['Payload Mass (kg)'] <= payload_mass_range[1])]
